...
def print_bond(atom_symbol, atom_coord, outfile):
        ...
        for j, atomB in enumerate(atom_coord):
            if j>i: 
                ...
                if bond_check(bond_length_AB):
                    ...
                    outfile.write(F'{atom_symbol[i]} to {atom_symbol[j]} is {bond_length_AB} \n')
                else:
                    print(F'{atom_symbol[i]} to {atom_symbol[j]} is {bond_length_AB} --> not bonded')
                    outfile.write(F'{atom_symbol[i]} to {atom_symbol[j]} is {bond_length_AB} --> not bonded \n')
            if j==i:
                print(F'{atom_symbol[i]} to {atom_symbol[j]} is the same atom')
                outfile.write(F'{atom_symbol[i]} to {atom_symbol[j]} is the same atom \n')
            if j<i:
                print(F'{atom_symbol[i]} to {atom_symbol[j]}  is evaluated')
                outfile.write(F'{atom_symbol[i]} to {atom_symbol[j]}  is evaluated \n')
...
if __name__ == "__main__": 
    ...
    print_bond(symbols, coord, outfile)
    outfile.close()